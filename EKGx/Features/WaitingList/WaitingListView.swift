            ...
            HStack {
                ...
                HStack(spacing: AppMetrics.spacing12) {
                    ...
                    if viewModel.hasDone {
                        Button(L10n.WaitingList.clearDone) {
                            withAnimation { viewModel.clearDone() }
                        }
                        .font(AppTypography.callout)
                    ...
                    Button { viewModel.openAddPatient() } label: {
                        HStack(spacing: 6) {
                            ...
                            Text(L10n.WaitingList.addPatient)
                                .font(AppTypography.callout)
                        }
...
private struct WaitingPatientRow: View {
    ...
    var body: some View {
        HStack(spacing: AppMetrics.spacing14) {
            ...
            // Status label + action stacked
            VStack(alignment: .trailing, spacing: 8) {
                Text(patient.status.displayTitle)
                    .font(AppTypography.captionBold)
                    .foregroundStyle(AppColors.textSecondary)
        ...
        case .waiting:
            Button(action: onStart) {
                HStack(spacing: 5) {
                    ...
                    Text(L10n.WaitingList.startEKG)
                        .font(AppTypography.captionBold)
                }
                .foregroundStyle(.white)
        ...
        case .inProgress:
            Button(action: onStart) {
                HStack(spacing: 5) {
                    ...
                    Text(L10n.WaitingList.continueEKG)
                        .font(AppTypography.captionBold)
                }
