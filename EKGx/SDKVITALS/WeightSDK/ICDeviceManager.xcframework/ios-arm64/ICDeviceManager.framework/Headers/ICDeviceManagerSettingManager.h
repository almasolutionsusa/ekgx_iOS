 ...
 * @param type 数据类型
 * @param userId 数据对应的用户ID，有些数据类型是不需要的，则填0就可以了
 * @param foodId 数据对应的食物ID
 * @param foodIndex 数据对应的食物序号
 * @param obj 数据，根据type来确定obj的类型，具体可以查看type的类型描述
 * @param callback
 */
 ...
 * 设置ECG
 * @param device 设备
 * @param enable 是否测量ECG
 * @param duration 时长
 * @param callback 回调
 */
- (void)setScaleECG:(ICDevice *)device enable:(BOOL)enable duartion:(NSUInteger)duration callback:(ICSettingCallback)callback;
 ...
 * @param callback 回调
 */
- (void)stopMeasureECG:(ICDevice *)device callback:(ICSettingCallback)callback;
